...
Local stdio MCP server that **mirrors** Knowledge Base markdown notes into
Notion and records the resulting Notion URL back into each note's YAML
frontmatter.
...
- A folder's **index note** is `<Folder>/<Folder>.md` (its basename equals the
  containing folder's basename). That note becomes the folder's Notion page.
...
1. <https://www.notion.so/my-integrations> → **New integration** (internal).
   Give it **Read content**, **Insert content**, and **Update content**
   capabilities.
2. Copy the **Internal Integration Secret** (`ntn_…`). Treat it like a password.
3. Open every target page/database in Notion → **⋯** menu → **Connections** →
   add your integration. Without this connection the API returns
...
Add to `~/Library/Application Support/Claude/claude_desktop_config.json` (or the
Claude Code equivalent) — see
...
Copy [`.env.example`](./.env.example) to `.env.development` and fill in your
...
See [ROADMAP.md](./ROADMAP.md) for planned work (image uploads, a publish diff
tool, backlink sync).
